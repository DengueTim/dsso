...
// Left to right. Takes R and t from CalibHessian.
EIGEN_STRONG_INLINE bool projectPointLR(const float u_pt, const float v_pt, const float idepth, CalibHessian *const&HCalib,
		float &drescale, float &u, float &v, float &Ku, float &Kv, Vec3f &KliP, float &new_idepth) {
	KliP = Vec3f((u_pt - HCalib->cxl()) * HCalib->fxli(), (v_pt - HCalib->cyl()) * HCalib->fyli(), 1);

	const Mat33f &R = HCalib->PRE_RTll_0;
	const Vec3f &t = HCalib->PRE_tTll_0;
