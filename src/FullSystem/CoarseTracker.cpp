	...
	for (int lvl = 1; lvl < pyrLevelsUsed; lvl++) {
		...
		for (int y = 0; y < hl; y++)
			for (int x = 0; x < wl; x++) {
				...
				weightSums_l[x + y * wl] = weightSums_lm[bidx] + weightSums_lm[bidx + 1] + weightSums_lm[bidx + wlm1]
...
void CoarseTracker::calcGSSSE(int lvl, Mat88 &H_out, Vec8 &b_out, const SE3 &refToNew, AffLight aff_g2l) {
	...
	__m128 a = _mm_set1_ps(
			(float) (AffLight::fromToVecExposure(lastRef->ab_exposure, newFrame->ab_exposure, lastRef_aff_g2l, aff_g2l)[0]));
	...
	for (int i = 0; i < n; i += 4) {
		...
		acc.updateSSE_eighted(_mm_mul_ps(id, dx), _mm_mul_ps(id, dy),
				...
				_mm_mul_ps(a, _mm_sub_ps(b0, _mm_load_ps(buf_warped_refColor + i))), minusOne, _mm_load_ps(buf_warped_residual + i),
...
bool CoarseTracker::trackNewestCoarse(FrameHessian *newFrameHessian, SE3 &lastToNew_out, AffLight &aff_g2l_out, int coarsestLvl,
		...
		for (int iteration = 0; iteration < maxIterations[lvl]; iteration++) {
			...
			if (debugPrint) {
				...
				printf("lvl %d, it %d (l=%f / %f) %s: %.3f->%.3f (%d -> %d) (|inc| = %f)! \t", lvl, iteration, lambda, extrapFac,
						(accept ? "ACCEPT" : "REJECT"), resOld[0] / resOld[1], resNew[0] / resNew[1], (int) resOld[1],
	...
	Vec2f relAff =
			AffLight::fromToVecExposure(lastRef->ab_exposure, newFrame->ab_exposure, lastRef_aff_g2l, aff_g2l_out).cast<float>();
